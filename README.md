...
## Exemple de prompt
#### Le premier prompt donné à Claude :
Je veux créer une application mobile : un jeu de plateforme comme mario bros en 2D avec comme personnage une boule avec des yeux qui avance. le code serait en java sur un téléphone android. Le jeu est format paysage et doit utiliser les capteurs du téléphone. le gyroscope pour avancer (le telephone est penché vers la droite) et reculer (le téléphone est penché vers la gauche) et enfin pour sauter les obstacles il doit utilier le micro, le joueur doit crier, plus le cri est fort plus le saut est haut. Il faudrait que le jeu ait un menu d'accueil, pouvoir faire pause et play, changer de niveau. Il faudrait une deuxième application de système d'édition de niveau qui permet à une personne de créer les niveaux sur ordinateur (avec le départ, l'arriver, un système de pièce et de compteur de pièce, de plateforme, de méchant, obstacle...) puis le niveau est ensuite lancable de l'application. attention à la fênetre, si le personnage tombe, touche un ennemi il perd. Lorsque le personnage arrive à la fin du niveau, il y a une fênetre qui dit "gagné" avec le nombre de pièces récoltées. Le jeu doit pouvoir recommencer à la fin d'une partie mais également dans le menu pause. Je veux un fichier apk directement et facilement lançable sur android. j'aimerais également avoir les tests en même temps que tu codes. stp merci
